var vfed = {
	'player': {
		...
		'cplayer': function(auto, live, trys, seek, take, urls, jump, pics) {
		...
		'aplayer': function(auto, live, trys, seek, take, urls, jump, pics) {
		...
		'eplayer': function(auto, live, trys, seek, take, urls, jump, pics) {
			var type = urls.indexOf('.m3u8') > -1 ? 'customHls' : 'auto';
			var player = new DPlayer({
				container: document.getElementById('video'),
				preload: 'auto',         //视频预加载（注意： 如果使用 autoplay 属性 preload 将被忽略。）
				live: live,
				video: {
					url: urls,
					type: type,
			...
			player.on('error', function() {
				vfed.player.dplayer(auto, live, trys, seek, take, urls, jump, pics);
			});
		},
		'dplayer': function(auto, live, trys, seek, take, urls, jump,  pics) {
			var player = new DPlayer({
				container: document.getElementById('video'),
				autoplay: auto,
				live: live,
